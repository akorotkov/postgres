 ...
 * space follows. At the end of the page (before special space), is an array
 * of uncompressed items; pd_upper points to that location. This layout is
 * compatible with the "standard" heap and index page layout described in
 * bufpage.h, so that we can e.g set buffer_std when writing WAL records:
 *
 * +----------------+-------------+-------------------+
...
#define GinDataLeafPageGetPostingList(page) \
	(GinPostingList *) ((PageGetContents(page) + sizeof(ItemPointerData)))
 ...
 * contents of the page (ginxlogInsertDataLeafCompressed). That replaces any
 * existing packed and unpacked items. Otherwise what follows is an array of
 * ItemPointers, inserted into the uncompressed area
 * (ginxlogInsertDataLeafUncompressed).
 */
#define INSERT_RECOMPRESS_FLAG (1<<15)
typedef struct
{
	uint16		length;
	...
	char		newdata[1];
} ginxlogInsertDataLeafCompressed;
 ...
 * records.) This is only used for data leaf pages, because entry items and
 * posting tree internal nodes are never removed.
 */
...
typedef struct GinBtreeData
{
	...
	bool		(*placeToPage) (GinBtree, Buffer, GinBtreeStack *, void *, BlockNumber, XLogRecData **, Page *, Page *);
