...
ginRedoInsertData(Buffer buffer, bool isLeaf, BlockNumber rightblkno, void *rdata)
{
	...
	if (isLeaf)
	{
		uint16 *firstfield = (uint16 *) rdata;

		Assert(GinPageIsLeaf(page));

		if (((*firstfield) & INSERT_RECOMPRESS_FLAG) != 0)
		{
			ginxlogInsertDataLeafCompressed *data = (ginxlogInsertDataLeafCompressed *) rdata;
			int			len = data->length & (~INSERT_RECOMPRESS_FLAG);
			Pointer		segment;

			/* Copy the new data to the right place */
			segment = ((Pointer) GinDataLeafPageGetPostingList(page))
				+ data->unmodifiedsize;
			memcpy(segment, data->newdata, len - data->unmodifiedsize);
		...
		else
		{
			...
			ginxlogInsertDataLeafUncompressed *data =
				(ginxlogInsertDataLeafUncompressed *) rdata;
			...
			/*
			 * we only allow uncompressed items to be inserted to new-style
			 * pages.
			 */
			Assert(GinPageIsCompressed(page));

			/* merge the old and new items into one list. */
			nnewitems = data->length / sizeof(ItemPointerData);
			newitems = data->newitems;

			olditems = GinDataLeafPageGetUncompressed(page, &nolditems);

			allitems = palloc((nnewitems + nolditems) * sizeof(ItemPointerData));
			nallitems = ginMergeItemPointers(allitems,
											 olditems, nolditems,
											 newitems, nnewitems);
			phdr->pd_upper = phdr->pd_special - nallitems * sizeof(ItemPointerData);
