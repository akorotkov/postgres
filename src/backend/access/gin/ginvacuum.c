					 ...
					 CompressedPostingList *cleaned, Size *newSize)
{
	...
	while (ptr < end && (nitems == 0 || i < nitems))
	{
			...
			if (!dst)
			{
				...
				*cleaned = dst;
				if (prev != src)
				{
					...
					dst += prev - src;
					writestate = ginPostingListEncodeBegin(&prevIptr);
				}
			}
			copying = true;
		}
		else
		{
			...
			if (copying)
				dst = ginPostingListEncode(dst, &iptr, &writestate);
			else
				prevIptr = iptr;
		}
		i++;
		...
		int nitem, CompressedPostingList *cleaned, Size *newSize)
{
	...
	for (i = 0; i < nitem; i++)
	{
		if (gvs->callback(items + i, gvs->callback_state))
		{
			...
			{
				/*
				 * First TID to be deleted: allocate memory for compressed
				...
				{
					for (j = 0; j < i; j++)
						ptr = ginPostingListEncode(ptr, items + j, &writestate);
				}
				copying = true;
			}
		}
		else
		{
			gvs->result->num_index_tuples += 1;
			if (copying)
				ptr = ginPostingListEncode(ptr, items + i, &writestate);
			remaining++;
		}
	}
