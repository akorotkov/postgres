 ...
 *
 * IDENTIFICATION
 *			src/backend/access/vodka/vodkapostinglist.c
 *-------------------------------------------------------------------------
 */
 ...
 * careful to not cause expansion e.g when mervodkag uncompressed items on the
 ...
 * use the minimum number of bits, but that would require chanvodkag the on-disk
 ...
 */
VodkaPostingList *
vodkaCompressPostingList(const ItemPointer ipd, int nipd, int maxsize,
					   int *nwritten)
{
	uint64		prev;
	int			totalpacked = 0;
	int			maxbytes;
	VodkaPostingList *result;
	unsigned char *ptr;
	...
	Assert(SizeOfVodkaPostingList(result) <= maxsize);

	/*
	 * Check that the encoded segment decodes back to the orivodkaal items.
	 */
#if defined (CHECK_ENCODING_ROUNDTRIP)
	if (assert_enabled)
	{
		int			ndecoded;
		ItemPointer tmp = vodkaPostingListDecode(result, &ndecoded);
		int			i;
 ...
 */
ItemPointer
vodkaPostingListDecode(VodkaPostingList *plist, int *ndecoded)
{
	return vodkaPostingListDecodeAllSegments(plist,
										   SizeOfVodkaPostingList(plist),
										   ndecoded);
}
 ...
 */
ItemPointer
vodkaPostingListDecodeAllSegments(VodkaPostingList *segment, int len, int *ndecoded_out)
{
	ItemPointer	result;
	int			nallocated;
	uint64		val;
	...
	while ((char *) segment < endseg)
	{
		...
		Assert(ndecoded == 0 || vodkaCompareItemPointers(&segment->first, &result[ndecoded - 1]) > 0);
		...
		segment = VodkaNextPostingListSegment(segment);
	}
 ...
 */
int
vodkaPostingListDecodeAllSegmentsToTbm(VodkaPostingList *ptr, int len,
									 TIDBitmap *tbm)
{
	int			ndecoded;
	ItemPointer	items;

	items = vodkaPostingListDecodeAllSegments(ptr, len, &ndecoded);
 ...
 */
ItemPointer
vodkaMergeItemPointers(ItemPointerData *a, uint32 na,
					 ...
					 int *nmerged)
{
	 ...
	 * each other.
	 */
	if (na == 0 || nb == 0 || vodkaCompareItemPointers(&a[na - 1], &b[0]) < 0)
	{
		memcpy(dst, a, na * sizeof(ItemPointerData));
		memcpy(&dst[na], b, nb * sizeof(ItemPointerData));
		*nmerged = na + nb;
	}
	else if (vodkaCompareItemPointers(&b[nb - 1], &a[0]) < 0)
	{
	...
	else
	{
		...
		{
			int			cmp = vodkaCompareItemPointers(aptr, bptr);
