 ...
 *
 * IDENTIFICATION
 *			src/backend/access/vodka/vodkabtree.c
 *-------------------------------------------------------------------------
 */
...
static void vodkaFindParents(VodkaBtree btree, VodkaBtreeStack *stack);
static bool vodkaPlaceToPage(VodkaBtree btree, VodkaBtreeStack *stack,
			   void *insertdata, BlockNumber updateblkno,
			   Buffer childbuf, VodkaStatsData *buildStats);
static void vodkaFinishSplit(VodkaBtree btree, VodkaBtreeStack *stack,
			   bool freestack, VodkaStatsData *buildStats);

/*
 * Lock buffer by needed method for search.
 */
static int
vodkaTraverseLock(Buffer buffer, bool searchMode)
{
	Page		page;
	int			access = VODKA_SHARE;
 ...
 */
VodkaBtreeStack *
vodkaFindLeafPage(VodkaBtree btree, bool searchMode)
{
	VodkaBtreeStack *stack;

	stack = (VodkaBtreeStack *) palloc(sizeof(VodkaBtreeStack));
	...
	for (;;)
	{
		...
		access = vodkaTraverseLock(stack->buffer, searchMode);

		/*
		 * If we're going to modify the tree, finish any incomplete splits we
		 * encounter on the way.
		 */
		if (!searchMode && VodkaPageIsIncompleteSplit(page))
			vodkaFinishSplit(btree, stack, false, NULL);
		...
		{
			BlockNumber rightlink = VodkaPageGetOpaque(page)->rightlink;

			if (rightlink == InvalidBlockNumber)
				/* rightmost page */
				break;

			stack->buffer = vodkaStepRight(stack->buffer, btree->index, access);
		...
		if (VodkaPageIsLeaf(page))	/* we found, return locked page */
			return stack;
		...
		{
			VodkaBtreeStack *ptr = (VodkaBtreeStack *) palloc(sizeof(VodkaBtreeStack));
 ...
 */
Buffer
vodkaStepRight(Buffer buffer, Relation index, int lockmode)
{
	Buffer		nextbuffer;
	Page		page = BufferGetPage(buffer);
	bool		isLeaf = VodkaPageIsLeaf(page);
	bool		isData = VodkaPageIsData(page);
	...
	if (isLeaf != VodkaPageIsLeaf(page) || isData != VodkaPageIsData(page))
		elog(ERROR, "right sibling of VODKA page is of different type");
	...
	if (VodkaPageIsDeleted(page))
		elog(ERROR, "right sibling of VODKA page was deleted");

	return nextbuffer;
}

void
freeVodkaBtreeStack(VodkaBtreeStack *stack)
{
	while (stack)
	{
		VodkaBtreeStack *tmp = stack->parent;
 ...
 */
static void
vodkaFindParents(VodkaBtree btree, VodkaBtreeStack *stack)
{
	Page		page;
	Buffer		buffer;
	BlockNumber blkno,
	...
	ptr = (VodkaBtreeStack *) palloc(sizeof(VodkaBtreeStack));
	...
	for (;;)
	{
		...
		if (VodkaPageIsIncompleteSplit(page))
		{
			Assert(blkno != btree->rootBlkno);
			ptr->blkno = blkno;
			ptr->buffer = buffer;
			/*
			 * parent may be wrong, but if so, the vodkaFinishSplit call will
			 * recurse to call vodkaFindParents again to fix it.
			 */
			ptr->parent = root;
			ptr->off = InvalidOffsetNumber;

			vodkaFinishSplit(btree, ptr, false, NULL);
		}
		...
		while ((offset = btree->findChildPtr(btree, page, stack->blkno, InvalidOffsetNumber)) == InvalidOffsetNumber)
		{
			...
			buffer = vodkaStepRight(buffer, btree->index, VODKA_EXCLUSIVE);
			page = BufferGetPage(buffer);
 ...
 * child page that was split. Its VODKA_INCOMPLETE_SPLIT flag will be cleared
 ...
 */
static bool
vodkaPlaceToPage(VodkaBtree btree, VodkaBtreeStack *stack,
			   void *insertdata, BlockNumber updateblkno,
			   Buffer childbuf, VodkaStatsData *buildStats)
{
	Page		page = BufferGetPage(stack->buffer);
	XLogRecData *payloadrdata;
	VodkaPlaceToPageRC rc;
	uint16		xlflags = 0;
	Page		childpage = NULL;
	Page		newlpage = NULL, newrpage = NULL;

	if (VodkaPageIsData(page))
		xlflags |= VODKA_INSERT_ISDATA;
	if (VodkaPageIsLeaf(page))
	{
		xlflags |= VODKA_INSERT_ISLEAF;
	...
	else if (rc == INSERTED)
	{
		...
		{
			VodkaPageGetOpaque(childpage)->flags &= ~VODKA_INCOMPLETE_SPLIT;
		...
		if (RelationNeedsWAL(btree->index))
		{
			...
			rdata[0].len = sizeof(vodkaxlogInsert);
			...
			if (childbuf != InvalidBuffer)
			{
				...
				BlockIdSet(&childblknos[1], VodkaPageGetOpaque(childpage)->rightlink);
			...
			recptr = XLogInsert(RM_VODKA_ID, XLOG_VODKA_INSERT, rdata);
	...
	else if (rc == SPLIT)
	{
		...
		rbuffer = VodkaNewBuffer(btree->index);
		...
		savedRightLink = VodkaPageGetOpaque(page)->rightlink;
		...
		if (childbuf != InvalidBuffer)
		{
			...
			data.rightChildBlkno = VodkaPageGetOpaque(childpage)->rightlink;
		}
		else
		...
		rdata[0].len = sizeof(vodkaxlogSplit);
		...
		if (stack->parent == NULL)
		{
			 ...
			 */
			lbuffer = VodkaNewBuffer(btree->index);
			...
			data.flags |= VODKA_SPLIT_ROOT;

			VodkaPageGetOpaque(newrpage)->rightlink = InvalidBlockNumber;
			VodkaPageGetOpaque(newlpage)->rightlink = BufferGetBlockNumber(rbuffer);

			/*
			 * Construct a new root page containing downlinks to the new left
			 * and right pages. (do this in a temporary copy first rather
			 * than overwriting the orivodkaal page directly, so that we can still
			 * abort gracefully if this fails.)
			 */
			newrootpg = PageGetTempPage(newrpage);
			VodkaInitPage(newrootpg, VodkaPageGetOpaque(newlpage)->flags & ~(VODKA_LEAF | VODKA_COMPRESSED), BLCKSZ);
		...
		else
		{
			...
			VodkaPageGetOpaque(newrpage)->rightlink = savedRightLink;
			VodkaPageGetOpaque(newlpage)->flags |= VODKA_INCOMPLETE_SPLIT;
			VodkaPageGetOpaque(newlpage)->rightlink = BufferGetBlockNumber(rbuffer);
		}
		 ...
		 * in. The orivodkaal page is still unchanged.
		 *
		 * If this is a root split, we also have a temporary page containing
		 * the new contents of the root. Copy the new left page to a
		 * newly-allocated block, and initialize the (orivodkaal) root page the
		...
		if (RelationNeedsWAL(btree->index))
		{
			...
			recptr = XLogInsert(RM_VODKA_ID, XLOG_VODKA_SPLIT, rdata);
	...
	{
		elog(ERROR, "unknown return code from VODKA placeToPage method: %d", rc);
 ...
 */
static void
vodkaFinishSplit(VodkaBtree btree, VodkaBtreeStack *stack, bool freestack,
			   VodkaStatsData *buildStats)
{
	 ...
	 */
	if (!freestack)
		elog(DEBUG1, "finishing incomplete split of block %u in vodka index \"%s\"",
			 stack->blkno, RelationGetRelationName(btree->index));

	/* this loop crawls up the stack until the insertion is complete */
	do
	{
		VodkaBtreeStack *parent = stack->parent;
		void	   *insertdata;
		BlockNumber updateblkno;

		/* search parent to lock */
		LockBuffer(parent->buffer, VODKA_EXCLUSIVE);
		 ...
		 * would fail.
		 */
		if (VodkaPageIsIncompleteSplit(BufferGetPage(parent->buffer)))
			vodkaFinishSplit(btree, parent, false, buildStats);
		...
		while ((parent->off = btree->findChildPtr(btree, page, stack->blkno, parent->off)) == InvalidOffsetNumber)
		{
			if (VodkaPageRightMost(page))
			{
				 ...
				 * plain search...
				 */
				LockBuffer(parent->buffer, VODKA_UNLOCK);
				vodkaFindParents(btree, stack);
				parent = stack->parent;
				Assert(parent != NULL);
				break;
			}

			parent->buffer = vodkaStepRight(parent->buffer, btree->index, VODKA_EXCLUSIVE);
		...
		updateblkno = VodkaPageGetOpaque(BufferGetPage(stack->buffer))->rightlink;
		done = vodkaPlaceToPage(btree, parent,
							  insertdata, updateblkno,
 ...
 * on whether this is an entry or data tree, vodkaInsertValue just passes it
 ...
 *
 * NB: the passed-in stack is freed, as though by freeVodkaBtreeStack.
 */
void
vodkaInsertValue(VodkaBtree btree, VodkaBtreeStack *stack, void *insertdata,
			   VodkaStatsData *buildStats)
{
	bool		done;

	/* If the leaf page was incompletely split, finish the split first */
	if (VodkaPageIsIncompleteSplit(BufferGetPage(stack->buffer)))
		vodkaFinishSplit(btree, stack, false, buildStats);

	done = vodkaPlaceToPage(btree, stack,
	...
	if (done)
	{
		...
		vodkaFinishSplit(btree, stack, true, buildStats);
}
